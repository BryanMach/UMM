  ...
  @section('content')
      <div class="container">
          <div class="row">
              <div class="col-md-9">
                  <div class="card">
                      <div class="card-header">datos Adicionales {{ $datosadicionale->id }}</div>
                      <div class="card-body">

                          <a href="{{ url('/admin/datos-adicionales') }}" title="Back"><button
                                  class="btn btn-warning btn-sm"><i class="fa fa-arrow-left" aria-hidden="true"></i>
                                  ATRAS</button></a>
                          <a href="{{ url('/admin/datos-adicionales/' . $datosadicionale->id . '/edit') }}"
                          ...
                          <form method="POST" action="{{ url('admin/datosadicionales' . '/' . $datosadicionale->id) }}"
                              accept-charset="UTF-8" style="display:inline">
                              {{ method_field('DELETE') }}
                              {{ csrf_field() }}
                              <button type="submit" class="btn btn-danger btn-sm" title="BORRAR datosAdicionale"
                              ...
                              <table class="table">
                                  <tbody>
                                      ...
                                      <tr>
                                          ...
                                          <td> {{ $datosadicionale->idArtefacto }} </td>
                                      </tr>
                                      <tr>
                                          <th> LUGAR </th>
                                          <td> {{ $datosadicionale->lugar }} </td>
                                      </tr>
                                      <tr>
                                          <th> MERCANCIA PELIGROSA </th>
                                          <td> {{ $datosadicionale->mercPelig }} </td>
                                      </tr>
                                  </tbody>
                              </table>
                          </div>

              ...
              <div class="card">
                  ...
                  <div class="card-body">
                        @switch($nivel)
                          ...
                          @default
                      @endswitch
                      <a href="{{ url('/admin/documentaciones/create') }}" class="btn btn-success btn-sm"
                          title="Agregar nuevo Documentacione">
                          <i class="fa fa-plus" aria-hidden="true"></i> Agregar
                      </a>

                      <form method="GET" action="{{ url('/admin/documentaciones') }}" accept-charset="UTF-8"
                              ...
                              <tbody>
                                  @foreach ($documentaciones as $item)
                                      <tr>
                                          ...
                                          <td>
                                              <a href="{{ url('/admin/documentaciones/' . $item->id) }}"
                                                  title="Ver Documentacione"><button class="btn btn-info btn-sm"><i
                                                          class="fa fa-eye" aria-hidden="true"></i> Ver</button></a>
                                              <a href="{{ url('/admin/documentaciones/' . $item->id . '/edit') }}"
                                                  title="Editar Documentacione"><button
                                                      class="btn btn-primary btn-sm"><i class="fa fa-pencil-square-o"
                                                          aria-hidden="true"></i>
                                                      Editar</button></a>

                                              <form method="POST"
                                                  action="{{ url('/admin/documentaciones' . '/' . $item->id) }}"
                                                  accept-charset="UTF-8" style="display:inline">
                                                  {{ method_field('DELETE') }}
                                                  {{ csrf_field() }}
                                                  <button type="submit" class="btn btn-danger btn-sm"
                                                      title="Borrar Documentacione"

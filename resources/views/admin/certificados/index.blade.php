                              ...
                              <tbody>
                                  @foreach ($certificados as $item)
                                      <tr>
                                          ...
                                          <td>
                                              <a href="{{ url('/admin/certificados/' . $item->id) }}"
                                                  title="View certificado"><button class="btn btn-info btn-sm"><i
                                                          class="fa fa-eye" aria-hidden="true"></i> View</button></a>
                                              <a href="{{ url('/admin/certificados/' . $item->id . '/edit') }}"
                                                  title="Edit certificado"><button class="btn btn-primary btn-sm"><i
                                                          class="fa fa-pencil-square-o" aria-hidden="true"></i>
                                                      Edit</button></a>

                                              <form method="POST"
                                                  action="{{ url('/admin/certificados' . '/' . $item->id) }}"
                                                  accept-charset="UTF-8" style="display:inline">
                                                  {{ method_field('DELETE') }}
                                                  {{ csrf_field() }}
                                                  <button type="submit" class="btn btn-danger btn-sm"
                                                      title="Delete certificado"

                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        @switch($nivel)
                            ...
                            @default
                        @endswitch
                        <a href="{{ url('/admin/servicios/create') }}" class="btn btn-success btn-sm"
                            title="Add New servicio">
                            <i class="fa fa-plus" aria-hidden="true"></i> AGREGAR
                        </a>

                        <form method="GET" action="{{ url('/admin/servicios') }}" accept-charset="UTF-8"
                                ...
                                <tbody>
                                    @foreach ($servicios as $item)
                                        <tr>
                                            ...
                                            <td>{{ $item->servicio }}</td>
                                            <td>
                                                {{--  <a href="{{ url('/admin/servicios/' . $item->id) }}"
                                                    title="View servicio"><button class="btn btn-info btn-sm"><i
                                                            class="fa fa-eye" aria-hidden="true"></i> VER</button></a> --}}
                                                <a href="{{ url('/admin/servicios/' . $item->id . '/edit') }}"
                                                    title="Edit servicio"><button class="btn btn-primary btn-sm"><i
                                                            class="fa fa-pencil-square-o" aria-hidden="true"></i>
                                                        EDITAR</button></a>

                                                <form method="POST"
                                                    action="{{ url('/admin/servicios' . '/' . $item->id) }}"

                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        @switch($nivel)
                          ...
                          @default
                      @endswitch
                        <a href="{{ url('/admin/servicios/create') }}" class="btn btn-success btn-sm" title="Add New servicio">
                        ...
                        <div class="table-responsive">
                            <table class="table">
                                <thead>
                                    <tr>
                                        <th>Nº</th><th>SERVICIO</th><th>OPCIONES</th>
                                    </tr>
                                </thead>
                                <tbody>
                                @foreach($servicios as $item)
                                    <tr>
                                        ...
                                        <td>{{ $item->servicio }}</td>
                                        <td>
                                            <a href="{{ url('/admin/servicios/' . $item->id) }}" title="View servicio"><button class="btn btn-info btn-sm"><i class="fa fa-eye" aria-hidden="true"></i> VER</button></a>
                                            <a href="{{ url('/admin/servicios/' . $item->id . '/edit') }}" title="Edit servicio"><button class="btn btn-primary btn-sm"><i class="fa fa-pencil-square-o" aria-hidden="true"></i> EDITAR</button></a>

                                            <form method="POST" action="{{ url('/admin/servicios' . '/' . $item->id) }}" accept-charset="UTF-8" style="display:inline">
                                                {{ method_field('DELETE') }}
                                                {{ csrf_field() }}
                                                <button type="submit" class="btn btn-danger btn-sm" title="Delete servicio" onclick="return confirm(&quot;Confirm delete?&quot;)"><i class="fa fa-trash-o" aria-hidden="true"></i> BORRAR</button>
                                            </form>
                                        </td>
                                    </tr>
                                @endforeach
